...
list2=Menu()
list2.add_command(label='Bold',command=new_file) #To create menus.
list2.add_command(label='Under Line',command=open_file)
list2.add_command(label='Right Margin',command=save_file)
list2.add_command(label='Center Margin',command=save_as)
list2.add_command(label='Exit',command=gui.quit)
mymenu.add_cascade(label='Format',menu=list1) #To create a file option.
gui.config(menu=mymenu)
